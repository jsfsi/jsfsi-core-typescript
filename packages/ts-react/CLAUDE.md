...
- **AuthProvider\<TUser\>** / **useAuth\<TUser\>()**: generic auth context. Decoupled from IoC — the consuming app passes behavior as callback props, not a service class. Props: `loader`, `onAuthChanged(callback) => unsubscribe`, and the eight method callbacks `onSignIn` / `onSignOut` / `onSignInWithEmailAndPassword` / `onSignUp` / `onSignUpWithEmailAndPassword` / `onSendPasswordResetEmail` / `onSendEmailVerification` / `onReloadUser`. The provider stores callbacks in refs so consumers don't have to memoize them. Exposes `{ currentUser, loading, signIn, signOut, signInWithEmailAndPassword, signUp, signUpWithEmailAndPassword, sendPasswordResetEmail, sendEmailVerification, reloadUser }`; each exposed method is wrapped with try/finally so `loading` always resets, even on throw. `reloadUser` additionally updates `currentUser` on success so `auth.currentUser.emailVerified` reflects the fresh state after the server-side verification. Context value is memoized. While `loading` is true the provider renders `<Loader />` instead of children.
...
- **FirebaseClient**: `AuthClient<User>` backed by `firebase/compat/auth`. Constructor takes `FirebaseConfig`; call `.initialize()` before use. Firebase is a peer dependency and is externalized from the built bundle (kept at ~8 kB gzipped). `sendEmailVerification()` acts on the currently signed-in Firebase user and returns `Fail(EmailVerificationFailure)` when nobody is signed in. `reloadUser()` calls `firebaseUser.reload()` (which mutates the instance in place) then remaps the user; returns `Ok(null)` when nobody is signed in so consumers can call it defensively.
...
- **User**: base user type. Includes `emailVerified: boolean` (sourced from `firebaseUser.emailVerified`). Generic `TUser` params throughout the package are constrained with `extends User`.
- **SignInFailure** / **SignUpFailure** / **PasswordResetEmailFailure** / **EmailVerificationFailure** / **ReloadUserFailure**: one class per file, each extending `Failure` with `constructor(public readonly error: unknown)`.
